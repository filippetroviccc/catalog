...
- Store layer: binary (bincode) load/save, atomic writes, ID counters, and in-memory views.
...
4. Analyze: reuse index scan results -> aggregate usage -> report top folders/files.
...
2. Walk root with `ignore::WalkBuilder` in parallel, `follow_links(false)`.
...
- Report top usage (roots, top folders, top files).
...
- Stable default plain output fields for `search`/`recent`: `path  size  YYYY-MM-DD`.
- Stable `--long` plain output fields for `search`/`recent`: `id  YYYY-MM-DD HH:MM:SS  size  kind  ext  status  root  path`.
- `--json` must be stable for scripting.
- Use exit code `0` on success, `1` on command/runtime error, `2` on CLI parse/usage error.
...
- Use atomic writes when persisting the binary snapshot.
...
- Command/runtime errors: return `1` with concise context.
- CLI parse/usage errors: return `2` (handled by clap).
- Permission errors: warn once per root or aggregate count.
- Storage errors: currently return `1` as command/runtime errors.

---

## Testing

- See `scope/testing.md` for the current automated coverage status.
