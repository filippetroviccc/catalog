...
* `catalog init [--preset macos-user-additions|macos-deep]`
    ...
    * Creates config + SQLite DB location.
* `catalog roots`
...
* `catalog index [--full] [--one-filesystem]`
    ...
    * Scan roots, update DB; `--full` forces full rescan.
...
* `roots(id, path, added_at, preset_name?)`
* `files(id, root_id, rel_path, abs_path, is_dir, is_symlink, size, mtime, status, last_indexed_at)`
* `tags(id, name UNIQUE)`
...
* Median `search` response: <100ms on typical DB sizes.
...
* Optional full-text indexing for text files using SQLite FTS5
...
If you want the next step, I’ll translate this PRD into an **implementation checklist + module structure** (commands, indexing pipeline, DB layer, ignore matcher) so you can start coding without architecture thrash.
