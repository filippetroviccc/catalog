...
`catalog init` defaults to `macos-full` for new configs. Users can select narrower presets explicitly.
...
* `catalog init [--preset macos-user-additions|macos-deep|macos-full]`
    ...
    * Creates config + binary store location.
* `catalog roots`
...
* `catalog search <query> [--ext ...] [--after ...] [--before ...] [--root ...] [--json]`
...
* `catalog analyze [path] [--top N] [--files N] [--json] [--raw] [--tui]`
    ...
    * Analyze disk usage and report largest folders/files.
    * Prefer reusing the index scan to avoid duplicate filesystem walks.
...
* Output:
    ...
    * Default plain: `path  size  YYYY-MM-DD`
    * `--long` plain: `id  YYYY-MM-DD HH:MM:SS  size  kind  ext  status  root  path`
    * `--json` for scripting.
...
* Update model:
    ...
    * Upsert entries keyed by `(root_id, rel_path)` each run.
    * Refresh `size`, `mtime`, and `last_seen_run` for seen entries.
* Deletions:
...
* Indexing should keep metadata/state incremental and deterministic across runs.
...
* Search + recent
* Config file + binary store

**V1.1**

* `catalog watch` with filesystem notifications (current implementation uses polling)
...
* CLI: `clap`
* Walking: `ignore::WalkBuilder` (gitignore-style excludes, parallel traversal)
* Storage: binary store on filesystem (no DB dependency) + JSON export
* Config: `toml` + `serde`
* Logging: `tracing`
